...
export default function PhaseBars({ phase, height = 52, compact = false }) {
  ...
  return (
    <div>
      ...
      <div className="mono" style={{ display: "flex", gap: 16, fontSize: 12, color: "var(--ink-soft)", marginTop: 6 }}>
        <span>R {r}</span>
        <span>Y {y}</span>
        <span>B {b}</span>
      </div>
      {!compact && (
        <div className="mono" style={{ fontSize: 11, marginTop: 4, color: imbalance > 15 ? "var(--critical)" : "var(--ink-mute)" }}>
