      ...
      {STAGES.map((s, i) => {
        ...
        return (
          <React.Fragment key={s.key}>
            <div style={{ flex: "none", width: 128, textAlign: "center" }}>
              <div
                style={{
              ...
              >
                <Icon size={19} color={active ? "#fff" : "var(--ink-mute)"} />
              </div>
              <div style={{ fontSize: 12.5, fontWeight: active ? 600 : 500, color: active ? "var(--ink)" : "var(--ink-soft)" }}>{s.label}</div>
              <div className="mute" style={{ fontSize: 10.5, marginTop: 2, lineHeight: 1.4 }}>{s.desc}</div>
            </div>
            {i < STAGES.length - 1 && (
              <div style={{ flex: "none", width: 28, height: 44, display: "flex", alignItems: "center", justifyContent: "center" }}>
